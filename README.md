...
Les SWE (Software Engineers) sont là pour aider, pas pour donner les réponses. Ils poseront des questions qui te guident vers la solution. Ils ne taperont pas les commandes à ta place. C'est volontaire, et ça reflète la façon dont les collègues seniors fonctionnent en environnement professionnel.
...
L'industrie de la cybersécurité repose sur la confiance. Les employeurs ont besoin de savoir que tu ne compromettras pas leurs clients. Les clients ont besoin de savoir que tu ne dépasseras pas le cadre de la mission. Cette confiance commence ici.


Ton environnement local
Avant de te connecter aux labs distants, tu as besoin d'un environnement local adapté. Les outils de sécurité sont conçus pour Linux. L'industrie tourne sous Linux. Ta configuration locale doit refléter cette réalité.
Pourquoi Linux ?
Les outils que tu utiliseras tout au long de ce programme (Nmap, Wireshark, Burp Suite, Metasploit, scripts personnalisés) sont conçus pour Linux en premier. Certains n'existent même pas sur Windows. D'autres fonctionnent mal sous macOS. Utiliser Linux élimine les problèmes de compatibilité et t'apprend l'environnement que tu utiliseras professionnellement.
